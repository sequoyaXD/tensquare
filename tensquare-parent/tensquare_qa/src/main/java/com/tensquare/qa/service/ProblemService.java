...
public class ProblemService {
	...
	/** 最新回答列表
	 * 	业务分析 :
	 * 		问题列表属于某一个标签 , 根据实际业务 , 应该按照回复日期降序排列 , 页面数据也需要分页
	 */
    public Page<Problem> newlist(String labelid, int page, int size) {
    	// 注意:spring data jpa的分页默认是从0开始的 ,所以这里要减1
		Page<Problem> problemPage =  problemDao.findNewListByLabelid(labelid,PageRequest.of(page-1,size));
		return  problemPage ;
    }

    // 热门回答列表
	// 业务分析 : 标签名查询 , 按回答数倒序排列 , 再分页显示
	...
	// 等待回答列表
	// 业务分析 : 标签名查询 , 回答人数为0 ,按创建时间倒序排列 , 再分页显示
	public Page<Problem> waitlist(String labelid, int page, int size) {
		Page<Problem> problemPage = problemDao.findWaitListBylabelid(labelid,PageRequest.of(page-1,size));
		return problemPage;
	}
}
